...
func TestTaskDraft_ParentField_RoundTrips(t *testing.T) {
	tests := []struct {
		name  string
		draft TaskDraft
	}{
		{name: "parent set", draft: TaskDraft{Subject: "ship", Assigned: "alice", Parent: "7"}},
		{name: "parent empty", draft: TaskDraft{Subject: "ship", Assigned: "alice"}},
		{name: "parent with state and deps", draft: TaskDraft{Subject: "ship", Assigned: "alice", Parent: "9", State: "in_progress", DependsOn: []string{"A", "B"}}},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.Parent != tt.draft.Parent {
				t.Errorf("Parent: got %q, want %q (rendered:\n%s)", got.Parent, tt.draft.Parent, rendered)
			}
		})
	}
}
